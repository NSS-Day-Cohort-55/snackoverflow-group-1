...
const LunchList = () => {
  MenuManger.getFoods().then((foodArray) => {
    const foundLunchs = foodArray.filter((foodItem) => foodItem.menuId === 1);
...
const DrinkList = () => {
  MenuManger.getFoods().then((foodArray) => {
    const foundDrinks = foodArray.filter((foodItem) => foodItem.menuId === 2);
...
const AppetizersList = () => {
  MenuManger.getFoods().then((foodArray) => {
    const foundAppetizers = foodArray.filter(
      (foodItem) => foodItem.menuId === 3
    );
...
const DessertsList = () => {
  MenuManger.getFoods().then((foodArray) => {
    const foundDesserts = foodArray.filter((foodItem) => foodItem.menuId === 4);
